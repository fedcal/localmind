...
class XaiLlmAdapterTest {

    @Mock
    private OpenAiChatModel chatModel;

    private XaiLlmAdapter adapter;

    @BeforeEach
    void setUp() {
        adapter = new XaiLlmAdapter("test-api-key", "https://api.x.ai/v1");
        ReflectionTestUtils.setField(adapter, "chatModel", chatModel);
    }

    @Test
    void getProvider_shouldReturnXai() {
        // when
        LlmProvider provider = adapter.getProvider();

        // then
        assertThat(provider).isEqualTo(LlmProvider.XAI);
    }
    ...
    void call_shouldInvokeChatModelAndReturnCorrectResponse() {
        ...
        ChatResponse chatResponse = buildMockChatResponse("Hello from Grok!", "grok-2", 10, 20, 30);
        ...
        assertThat(response.getContent()).isEqualTo("Hello from Grok!");
        assertThat(response.getModel()).isEqualTo("grok-2");
        assertThat(response.getProvider()).isEqualTo(LlmProvider.XAI);
    }
    ...
    void call_shouldMapTokenUsageCorrectly() {
        ...
        ChatResponse chatResponse = buildMockChatResponse("Done", "grok-2", 15, 25, 40);
        when(chatModel.call(any(Prompt.class))).thenReturn(chatResponse);
    ...
    void call_shouldConvertMessageRolesCorrectly() {
        ...
        ChatResponse chatResponse = buildMockChatResponse("Response", "grok-2", 5, 10, 15);
        when(chatModel.call(any(Prompt.class))).thenReturn(chatResponse);
    ...
    @Test
    void call_shouldHandleToolRoleAsUserMessage() {
        // given
        LlmRequest request = LlmRequest.builder()
                .messages(List.of(
                        ChatMessage.builder()
        ...
        ChatResponse chatResponse = buildMockChatResponse("Processed", "grok-2", 5, 10, 15);
        when(chatModel.call(any(Prompt.class))).thenReturn(chatResponse);
        ...
        verify(chatModel).call(promptCaptor.capture());
        Prompt captured = promptCaptor.getValue();
        assertThat(captured.getInstructions()).hasSize(1);
        assertThat(captured.getInstructions().get(0)).isInstanceOf(UserMessage.class);
    }
