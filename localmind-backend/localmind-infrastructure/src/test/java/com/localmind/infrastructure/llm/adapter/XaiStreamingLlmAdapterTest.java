...
class XaiStreamingLlmAdapterTest {

    @Mock
    private OpenAiChatModel chatModel;

    private XaiStreamingLlmAdapter adapter;

    @BeforeEach
    void setUp() {
        adapter = new XaiStreamingLlmAdapter("test-api-key", "https://api.x.ai/v1");
    ...
    void streamResponse_shouldStreamTokensAndComplete() {
        ...
        ChatResponse finalChunk = buildStreamChunk(" Grok", 10, 20, 30);
        ...
        assertThat(receivedTokens).containsExactly("Hello", " from", " Grok");
    ...
    void streamResponse_shouldCallOnErrorWhenFluxErrors() {
        ...
        RuntimeException expectedError = new RuntimeException("xAI stream error");
        ...
        assertThat(receivedError.get().getMessage()).isEqualTo("xAI stream error");
    }
    ...
    void streamResponse_shouldCallOnErrorWhenExceptionThrownBeforeStream() {
        ...
        when(chatModel.stream(any(Prompt.class))).thenThrow(new RuntimeException("Connection refused"));
        ...
        // then
        assertThat(receivedError.get()).isNotNull();
        assertThat(receivedError.get().getMessage()).isEqualTo("Connection refused");
    }
