...
class DeepSeekLlmAdapterTest {
    ...
    private DeepSeekLlmAdapter adapter;

    @BeforeEach
    void setUp() {
        adapter = new DeepSeekLlmAdapter("test-api-key", "https://api.deepseek.com");
    ...
    void call_shouldInvokeChatModelAndReturnCorrectResponse() {
        ...
        ChatResponse chatResponse = buildMockChatResponse("Hello from DeepSeek!", "deepseek-chat", 10, 20, 30);
        ...
        assertThat(response.getContent()).isEqualTo("Hello from DeepSeek!");
        assertThat(response.getModel()).isEqualTo("deepseek-chat");
        assertThat(response.getProvider()).isEqualTo(LlmProvider.DEEPSEEK);
    }
    ...
    void call_shouldMapTokenUsageCorrectly() {
        ...
        ChatResponse chatResponse = buildMockChatResponse("Done", "deepseek-chat", 15, 25, 40);
        when(chatModel.call(any(Prompt.class))).thenReturn(chatResponse);
    ...
    void call_shouldConvertMessageRolesCorrectly() {
        ...
        ChatResponse chatResponse = buildMockChatResponse("Response", "deepseek-coder", 5, 10, 15);
        when(chatModel.call(any(Prompt.class))).thenReturn(chatResponse);
    ...
    void call_shouldHandleToolRoleAsUserMessage() {
        ...
        ChatResponse chatResponse = buildMockChatResponse("Processed", "deepseek-chat", 5, 10, 15);
        when(chatModel.call(any(Prompt.class))).thenReturn(chatResponse);
