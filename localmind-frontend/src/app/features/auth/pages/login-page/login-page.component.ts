        ...
        @if (authService.needsSetup()) {
          ...
          <form (ngSubmit)="onSetup()">
            <div class="form-group">
              ...
              <input type="password" [(ngModel)]="password" name="password" #setupPwd="ngModel"
                     [placeholder]="'AUTH.PASSWORD_PLACEHOLDER' | translate" required minlength="4">
              @if (setupPwd.invalid && setupPwd.touched) {
                <span class="form-error">
                  @if (setupPwd.errors?.['required']) { {{ 'VALIDATION.REQUIRED' | translate }} }
                  @else if (setupPwd.errors?.['minlength']) { {{ 'VALIDATION.PASSWORD_MIN_LENGTH' | translate }} }
                </span>
              }
            </div>
            <div class="form-group">
              <label>{{ 'AUTH.CONFIRM_PASSWORD' | translate }}</label>
              <input type="password" [(ngModel)]="confirmPassword" name="confirmPassword" #confirmPwd="ngModel"
                     [placeholder]="'AUTH.CONFIRM_PLACEHOLDER' | translate" required>
              @if (confirmPwd.touched && password && confirmPassword && password !== confirmPassword) {
                <span class="form-error">{{ 'VALIDATION.PASSWORD_MISMATCH' | translate }}</span>
              }
            </div>
            @if (error()) {
              <div class="error-msg">{{ error() }}</div>
            }
            <button type="submit" [disabled]="loading() || !password || password.length < 4 || password !== confirmPassword" class="btn-primary">
          ...
          <form (ngSubmit)="onLogin()">
            <div class="form-group">
              <label>{{ 'AUTH.PASSWORD' | translate }}</label>
              <input type="password" [(ngModel)]="password" name="password" #loginPwd="ngModel"
                     [placeholder]="'AUTH.PASSWORD_PLACEHOLDER' | translate" required
                     (keydown.enter)="onLogin()">
              @if (loginPwd.invalid && loginPwd.touched) {
