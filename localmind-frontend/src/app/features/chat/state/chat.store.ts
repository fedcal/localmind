...
export class ChatStore {
  ...
  private _advancedMode = signal<boolean>(typeof localStorage !== 'undefined' && localStorage.getItem('localmind-chat-mode') === 'advanced');
  ...
  toggleAdvancedMode() {
    this._advancedMode.update(v => {
      const newVal = !v;
      localStorage.setItem('localmind-chat-mode', newVal ? 'advanced' : 'simple');
      return newVal;
    });
  }
  ...
  setStreaming(streaming: boolean) {
    this._isStreaming.set(streaming);
  }
