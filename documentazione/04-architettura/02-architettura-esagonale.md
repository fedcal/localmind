...
- **Domain Services**: servizi che implementano logica di business complessa che non appartiene a una singola entita' (LlmGatewayService, DocumentService, ChunkingService, CostTrackingService, AutomationService, AgentService)
...
- **Ports In (Use Case)**: interfacce che il mondo esterno (controllers) invoca per interagire con il dominio
  ...
  - `AutomationUseCase`: gestione automazioni
...
- **Ports Out (SPI)**: interfacce che il dominio invoca per accedere a risorse esterne
  ...
  - `WebhookRepository`: persistenza webhook
  - `WebhookClientPort`: invio webhook HTTP
