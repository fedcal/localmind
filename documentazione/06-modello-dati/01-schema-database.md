...
Il database di LocalMind e' composto da 6 tabelle PostgreSQL, create e gestite tramite migrazioni Flyway. Lo schema e' progettato per supportare le funzionalita' core della piattaforma: gestione documenti, conversazioni chat, tracciamento dell'utilizzo LLM, configurazione cartelle e automazioni webhook.
...
|-------------|----------------|----------|----------------------|---------------------------------------|
| `id`        | `UUID`         | NO       | `gen_random_uuid()`  | Identificatore univoco                |
...
| `metadata`  | `JSONB`        | SI       | -                    | Metadati aggiuntivi in formato JSON   |
| `created_at`| `TIMESTAMP`    | NO       | `NOW()`              | Data/ora di creazione                 |
| `updated_at`| `TIMESTAMP`    | NO       | `NOW()`              | Data/ora ultimo aggiornamento         |
...
Il campo `metadata` e' di tipo JSONB e consente lo storage flessibile di metadati eterogenei. Esempi di contenuto:
...
| `cost`              | `DOUBLE PRECISION` | NO       | `0`                  | Costo stimato della chiamata          |
| `latency_ms`        | `BIGINT`           | NO       | `0`                  | Latenza in millisecondi               |
| `timestamp`         | `TIMESTAMP`        | NO       | `NOW()`              | Data/ora della chiamata               |
...
|-------------------|----------------|----------|----------------------|---------------------------------------|
| `id`              | `UUID`         | NO       | `gen_random_uuid()`  | Identificatore univoco                |
| `conversation_id` | `UUID`         | NO       | -                    | Riferimento alla conversazione        |
...
- **Chiavi primarie**: `id` di tipo `UUID` per tutte le tabelle.
- **Timestamp**: tipo `TIMESTAMP` senza timezone, con `DEFAULT NOW()` per le colonne di audit.

### Tipi di dato

- **Identificatori**: `UUID` con generazione lato database tramite `gen_random_uuid()`.
...
- **JSON**: `JSONB` per dati semi-strutturati (metadati documento).
- **Booleani**: `BOOLEAN` con `DEFAULT TRUE` o `DEFAULT FALSE`.
- **Numerici**: `INTEGER`, `BIGINT`, `DOUBLE PRECISION` in base alla precisione richiesta.
