export const content = `# User Guide - LLM Provider Settings
...
All provider configuration form fields have inline validation:

| Field | Validation | Error message |
|---|---|---|
| Provider name | Required, min. 2 characters | "Required field" / "Minimum 2 characters" |
| Base URL | Required | "Required field" |
| API Key | Required, min. 10 characters | "Required field" / "Minimum 10 characters" |

Invalid fields display a red border and an error message below the field when the user touches them. Valid fields display a green border.

---

## Webhooks

The Webhooks section (accessible from Settings > Webhooks in the sidebar) allows you to configure automatic notifications to external services.

### Configuration

Each webhook requires:
- **Name**: webhook identifier (min. 2 characters)
- **URL**: destination HTTP/HTTPS endpoint
- **Event type**: the event that triggers the webhook
...
| Type | Description |
|---|---|
| DOCUMENT_INDEXED | A document has been successfully indexed |
| DOCUMENT_FAILED | Document indexing has failed |
| NEW_FILE | A new file has been detected in a monitored folder |
| CHAT_COMPLETED | A chat conversation has been completed |
| SCHEDULED | Scheduled event |

### Actions

- **Test**: sends a test request to the webhook to verify the connection
- **Edit**: modifies the webhook configuration
- **Delete**: removes the webhook
- **Active toggle**: enables/disables the webhook without deleting it
`;
