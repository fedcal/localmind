export const content = `# Backend Technology Stack
...
17. [Automation and Webhooks](#17-automation-and-webhooks)
18. [Dependency Summary Table](#18-dependency-summary-table)
...
LocalMind supports LLM response streaming via Server-Sent Events (SSE) using Spring MVC's \`SseEmitter\`.

**Architecture**:
- \`StreamingChatUseCase\` (port in): interface for streaming chat
- \`StreamingLlmClient\` (port out): interface for streaming adapters, returns \`Flux<String>\`
- \`StreamingChatController\`: endpoint \`POST /api/v1/chat/stream\` with \`SseEmitter\` (5 min timeout)
- Adapters: \`OllamaStreamingLlmAdapter\`, \`OpenAiStreamingLlmAdapter\`, \`AnthropicStreamingLlmAdapter\`

Each adapter uses Spring AI 1.0.0's \`StreamingChatModel.stream()\` to obtain a \`Flux<ChatResponse>\` from the provider and converts it into a \`Flux<String>\` (text tokens).

**SSE Event Sequence**:
1. \`event: conversation\` - conversation ID
2. \`event: token\` - text token (repeated N times)
3. \`event: metadata\` - model, provider, token usage
4. \`event: done\` - completion

**Notes**: tool calling is disabled during streaming; RAG context is prepared before the stream begins.

---

## 17. Automation and Webhooks

The webhook system allows notifying external services when events occur.

**Domain**:
- \`AutomationService\`: domain service managing webhook CRUD and event dispatch
- \`AutomationUseCase\` (port in): CRUD + trigger interface
- \`WebhookRepository\` (port out): webhook persistence
- \`WebhookClientPort\` (port out): HTTP webhook invocation

**Infrastructure**:
- \`N8nWebhookClient\`: WebhookClientPort implementation using \`WebClient\` (spring-boot-starter-webflux)
- \`WebhookRepositoryAdapter\`: domain/entity mapping for the \`webhooks\` table

**API**:
- \`WebhookController\`: REST CRUD on \`/api/v1/webhooks\` + test endpoint

**Event Types**: \`NEW_FILE\`, \`DOCUMENT_INDEXED\`, \`DOCUMENT_FAILED\`, \`CHAT_COMPLETED\`, \`SCHEDULED\`

---

## 18. Dependency Summary Table
