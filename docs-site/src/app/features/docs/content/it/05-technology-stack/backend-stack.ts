export const content = `# Stack Tecnologico Backend
...
15. [Maven](#15-maven)
16. [SSE Streaming Chat](#16-sse-streaming-chat)
17. [Automazione e Webhooks](#17-automazione-e-webhooks)
18. [Tabella Riepilogativa Dipendenze](#18-tabella-riepilogativa-dipendenze)
...
## 16. SSE Streaming Chat

LocalMind supporta lo streaming delle risposte LLM tramite Server-Sent Events (SSE) usando \`SseEmitter\` di Spring MVC.

**Architettura**:
- \`StreamingChatUseCase\` (port in): interface per chat streaming
- \`StreamingLlmClient\` (port out): interface per adapter streaming, ritorna \`Flux<String>\`
- \`StreamingChatController\`: endpoint \`POST /api/v1/chat/stream\` con \`SseEmitter\` (timeout 5 min)
- Adapter: \`OllamaStreamingLlmAdapter\`, \`OpenAiStreamingLlmAdapter\`, \`AnthropicStreamingLlmAdapter\`

Ogni adapter usa \`StreamingChatModel.stream()\` di Spring AI 1.0.0 per ottenere un \`Flux<ChatResponse>\` dal provider e lo converte in \`Flux<String>\` (token di testo).

**Sequenza eventi SSE**:
1. \`event: conversation\` - ID conversazione
2. \`event: token\` - token di testo (ripetuto N volte)
3. \`event: metadata\` - modello, provider, token usage
4. \`event: done\` - completamento

**Note**: il tool calling e' disabilitato in streaming; il contesto RAG viene preparato prima dello stream.

---

## 17. Automazione e Webhooks

Il sistema di webhooks consente di notificare servizi esterni al verificarsi di eventi.

**Domain**:
- \`AutomationService\`: servizio domain che gestisce CRUD webhook e dispatch eventi
- \`AutomationUseCase\` (port in): interface CRUD + trigger
- \`WebhookRepository\` (port out): persistenza webhook
- \`WebhookClientPort\` (port out): invocazione HTTP webhook

**Infrastructure**:
- \`N8nWebhookClient\`: implementazione WebhookClientPort usando \`WebClient\` (spring-boot-starter-webflux)
- \`WebhookRepositoryAdapter\`: mapping domain/entity per tabella \`webhooks\`

**API**:
- \`WebhookController\`: REST CRUD su \`/api/v1/webhooks\` + endpoint test

**Tipi evento**: \`NEW_FILE\`, \`DOCUMENT_INDEXED\`, \`DOCUMENT_FAILED\`, \`CHAT_COMPLETED\`, \`SCHEDULED\`

---

## 18. Tabella Riepilogativa Dipendenze
