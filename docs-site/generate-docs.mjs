...
async function buildLinkMap(enFolders, itFolderMap) {
  for (const folder of enFolders) {
    ...
    try {
      ...
      const enFiles = entries.filter(e => e.isFile() && extname(e.name).toLowerCase() === '.md')
        .map(e => e.name).sort();
    ...
    if (itDir) {
      try {
        ...
        const enFiles = enEntries.filter(e => e.isFile() && extname(e.name).toLowerCase() === '.md')
          .map(e => e.name).sort();

        const itEntries = await readdir(itDir, { withFileTypes: true });
        const itFiles = itEntries.filter(e => e.isFile() && extname(e.name).toLowerCase() === '.md')
          .map(e => e.name).sort();
...
async function processCategory(enFolderName, enSourceDir, itSourceDir, itFolderBasename) {
  ...
  try {
    const entries = await readdir(enSourceDir, { withFileTypes: true });
    enFiles = entries.filter(e => e.isFile() && extname(e.name).toLowerCase() === '.md')
      .map(e => e.name)
      .sort();
  } catch (err) {
  ...
  if (itSourceDir) {
    try {
      ...
      itFiles = entries.filter(e => e.isFile() && extname(e.name).toLowerCase() === '.md')
        .map(e => e.name)
        .sort();
    } catch (err) {
